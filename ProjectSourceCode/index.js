...
async function insertPlaylists(playlistJson,username){
  console.log("Inside insertPlaylists()")
    ...
    for (i=0;i<playlistCount;i++){
        ...
        var img="https://media.istockphoto.com/id/1431567498/vector/vector-illustration-of-musical-notes-on-white-background.jpg?s=612x612&w=0&k=20&c=E4Qx8E7OJm-itMPylpaZhNIU8mkJQt5XctWlKLLa1I8="
...
app.get('/playlistsHomePage', async (req, res) => {  
  ...
  .then(async result => {
    ...
    //another query here
    // Before accessing 'profile_pic', check if the object is not undefined
    if (object !== undefined) {
      var profilePic = object.profile_pic;
    }

    // Before accessing 'images[0].url', check if 'images' is not null
